...
### 3. 클라우드 대시보드(GCP)와 내 로컬 PC 워커 연동하기 (터널 사용)
구글 클라우드(HTTPS)에 배포된 대시보드에서 내 개인 PC(로컬)의 워커로 핑 명령을 위임하려면, 외부 인바운드 접근 차단을 우회하기 위해 로컬 워커와 터널 프로그램이 **모두 동시에 실행 중**이어야 합니다.

1. **로컬 워커 구동**:
   반드시 워커 프로그램이 로컬 PC에서 9000번 포트로 계속 실행 중이어야 합니다:
   ```bash
   python3 worker.py --port 9000 --host 0.0.0.0
   ```
2. **로컬 터널 활성화 (새 터미널 창)**:
   다른 터미널 창을 열고 아래 명령어로 9000번 포트를 임시 외부 공인 주소로 터널링해 줍니다:
   ```bash
   npx localtunnel --port 9000
   ```
   * 실행 후 콘솔에 출력되는 주소(예: `your url is: https://xxxx.loca.lt`)를 복사합니다.
3. **대시보드 UI 입력**:
   * **IP**: 터널 주소에서 `https://`를 제외한 도메인 부분만 입력 (예: `xxxx.loca.lt`)
   * **Port**: **`443`** 기입 (터널 도메인은 HTTPS 규격을 사용하므로, 9000이 아닌 표준 SSL 포트 **443**을 적어야 백엔드 프록시가 정상 연동됩니다.)
